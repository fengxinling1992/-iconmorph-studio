...
export default function Home() {
  ...
  const normalizedExtrusionAngle = ((params.extrusionAngle % 360) + 360) % 360;
  const usesTopEdge = normalizedExtrusionAngle >= 90 && normalizedExtrusionAngle <= 270;
  const secondaryFaceLabel = usesTopEdge ? "顶边面" : "底边面";
  const secondaryFacePosition = usesTopEdge ? "顶边" : "底边";
    ...
    <div className="app-shell">
      ...
      <main className="workspace">
        ...
        <aside className="control-rail">
          ...
          {selectedStyle === "extrude" && <><div className="parameter-group"><div className="group-title"><WandSparkles size={15}/><span>挤出结构</span></div><SliderField label="挤出厚度" value={params.extrusion} min={4} max={42} suffix=" px" onChange={(value) => updateParam("extrusion", value)} /><SliderField label="挤出角度" value={params.extrusionAngle} min={0} max={360} suffix="°" onChange={(value) => updateParam("extrusionAngle", value)} /></div><div className="parameter-group face-color-group"><div className="group-title"><Layers3 size={15}/><span>融合双分面配色</span></div><p>当前为{secondaryFacePosition}挤出：90°–270° 自动切至顶边，其余角度使用底边；侧面与{secondaryFaceLabel}连续衔接。</p><div className="face-color-grid"><ColorField label="正面" color={params.frontColor} onChange={(value) => updateParam("frontColor", value)} /><ColorField label="侧面" color={params.sideColor} onChange={(value) => updateParam("sideColor", value)} /><ColorField label={secondaryFaceLabel} color={params.bottomColor} onChange={(value) => updateParam("bottomColor", value)} /></div></div></>}
          {selectedStyle === "scene" && <><div className="parameter-group"><div className="group-title"><Palette size={15}/><span>主体颜色</span></div><div className="color-row"><ColorField label="起始色" color={params.primary} onChange={(value) => updateParam("primary", value)} /><ColorField label="结束色" color={params.secondary} onChange={(value) => updateParam("secondary", value)} /></div><SliderField label="渐变角度" value={params.angle} min={0} max={360} suffix="°" onChange={(value) => updateParam("angle", value)} /></div><div className="parameter-group"><div className="group-title"><WandSparkles size={15}/><span>实体与玻璃</span></div><SliderField label="挤出厚度" value={params.extrusion} min={4} max={42} suffix=" px" onChange={(value) => updateParam("extrusion", value)} /><SliderField label="挤出角度" value={params.extrusionAngle} min={0} max={360} suffix="°" onChange={(value) => updateParam("extrusionAngle", value)} /><SliderField label="材质透明度" value={params.opacity} min={20} max={94} suffix="%" onChange={(value) => updateParam("opacity", value)} /><SliderField label="磨砂模糊" value={params.blur} min={0} max={24} suffix=" px" onChange={(value) => updateParam("blur", value)} /><SliderField label="边缘高光" value={params.highlight} min={0} max={100} suffix="%" onChange={(value) => updateParam("highlight", value)} /></div><div className="parameter-group face-color-group"><div className="group-title"><Layers3 size={15}/><span>融合双分面配色</span></div><p>当前为{secondaryFacePosition}挤出；90°–270° 自动切至顶边，其余角度使用底边。两面连续相接且可分别调整颜色。</p><div className="face-color-grid"><ColorField label="侧面" color={params.sideColor} onChange={(value) => updateParam("sideColor", value)} /><ColorField label={secondaryFaceLabel} color={params.bottomColor} onChange={(value) => updateParam("bottomColor", value)} /></div></div><div className="parameter-group scene-assets"><div className="group-title"><Clapperboard size={15}/><span>场景素材</span></div><p>默认使用底座、玻璃球、方块、飘带与环绕套件；所有素材按同一等轴透视编排。</p><button onClick={() => baseInput.current?.click()} className="scene-upload"><span className="scene-swatch base-swatch"/><div><strong>{params.sceneBase ? "已自定义底座" : "默认底座"}</strong><small>上传 SVG / PNG 覆盖</small></div><Upload size={15}/></button><button onClick={() => decorInput.current?.click()} className="scene-upload"><span className="scene-swatch decor-swatch"/><div><strong>{params.sceneDecor ? "已自定义装饰" : "默认装饰套件"}</strong><small>上传 SVG / PNG 覆盖</small></div><Upload size={15}/></button><input ref={baseInput} className="visually-hidden" type="file" accept="image/svg+xml,image/png" onChange={(event) => readSceneAsset(event, "sceneBase")} /><input ref={decorInput} className="visually-hidden" type="file" accept="image/svg+xml,image/png" onChange={(event) => readSceneAsset(event, "sceneDecor")} /></div></>}
